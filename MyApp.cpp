...
bool CMyApp::Init() {
  ...
  auto material_array = std::to_array<Material>({
...
void CMyApp::setUpBuffers() {
  constexpr auto rectangleBuffer =
      std::to_array<glm::vec2>({{-1, -1}, {1, -1}, {-1, 1}, {1, 1}});

  constexpr auto rectangleIndex = std::to_array<GLushort>({
      0U,
      1U,
      2U,
      2U,
...
void CMyApp::renderImgui() {
  ...
  for (int i = 0; i < circlesBuffer.size(); ++i) {
    ...
    ImGui::SliderFloat3(fmt::format("##{}", i).c_str(),
                        glm::value_ptr(circlesBuffer[i].center), -50.f, 50.f);
    ImGui::SliderFloat(fmt::format("##{}", i).c_str(),
                       &(circlesBuffer[i].radius), 1.f, 20.f);
  }

  ImGui::Text("exe : %f, %f, %f", m_camera.GetEye().x, m_camera.GetEye().y,
              m_camera.GetEye().z);
  ImGui::Text("at : %f, %f, %f", m_camera.GetAt().x, m_camera.GetAt().y,
              m_camera.GetAt().z);
  ImGui::Text("up : %f, %f, %f", m_camera.GetUp().x, m_camera.GetUp().y,
              m_camera.GetUp().z);
