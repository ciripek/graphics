...
public:
    ...
    explicit DSATextures(int size): size(size), ids(std::make_unique<GLuint[]>(size)) {
    ...
    constexpr GLuint& operator[](std::size_t i){
        return ids[i];
    }

    void AttachFromFile(const std::filesystem::path &name, bool generateMipMap = true, int index = 0) {
        ...
        const Uint32 sdl_format = loaded_img->format->BytesPerPixel == 3 ? SDL_PIXELFORMAT_RGB24 : SDL_PIXELFORMAT_RGBA32;
        ...
        glTextureSubImage2D(ids[index], 0, 0, 0, loaded_img->w, loaded_img->h, source_format, GL_UNSIGNED_BYTE, loaded_img->pixels);

        if (generateMipMap) glGenerateTextureMipmap(ids[index]);
