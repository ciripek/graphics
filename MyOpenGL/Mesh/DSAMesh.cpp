...
void DSAMesh::initBuffers() {
  ...
  glNamedBufferStorage(vertexBuffer, sizeof(vertex) * vertices.size(),
                       vertices.data(), 0);

  glCreateBuffers(1, &indexBuffer);
  glNamedBufferStorage(indexBuffer, sizeof(GLuint) * indices.size(),
                       indices.data(), 0);
}
...
void DSAMesh::draw(GLuint vertexArrayObject,
                   ShaderProgram& fragment,
                   const std::unordered_map<std::string, GLuint>& textureMap,
                   const std::vector<tinyobj::material_t>& materials) {
  glBindVertexArray(vertexArrayObject);

  glVertexArrayVertexBuffer(vertexArrayObject, 0, vertexBuffer, 0,
                            sizeof(vertex));
  glVertexArrayElementBuffer(vertexArrayObject, indexBuffer);

  fragment.setTexture(materials[material_index].diffuse_texname, 0,
                      textureMap.at(materials[material_index].diffuse_texname));
