...
void Model::loadShape(const tinyobj::attrib_t &attrib, const tinyobj::shape_t &shape, int index) {
...
void Model::loadTextures(const std::vector<tinyobj::material_t> &materials, const std::filesystem::path &name) {
...
void Model::draw(ShaderProgram &program) {
    for (int i = 0; i < buffers.getBufferCount(); i += 2) {
        ...
        glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(count[i + 1]), GL_UNSIGNED_INT, nullptr);
