...
/*
        ...
        Convenience conversion functions to simplify the creation of
   ShaderObject objects using the _vs, _tcs, _tes, _gs, _fs, and _comp suffices.

*/
using TypeSourcePair = std::pair<GLuint, std::string>;

inline TypeSourcePair operator"" _vs(const char* str, size_t len) {
  return std::make_pair(GL_VERTEX_SHADER, std::string{str, len});
}
inline TypeSourcePair operator"" _tcs(const char* str, size_t len) {
  return std::make_pair(GL_TESS_CONTROL_SHADER, std::string{str, len});
}
inline TypeSourcePair operator"" _tes(const char* str, size_t len) {
  return std::make_pair(GL_TESS_EVALUATION_SHADER, std::string{str, len});
}
inline TypeSourcePair operator"" _gs(const char* str, size_t len) {
  return std::make_pair(GL_GEOMETRY_SHADER, std::string{str, len});
}
inline TypeSourcePair operator"" _fs(const char* str, size_t len) {
  return std::make_pair(GL_FRAGMENT_SHADER, std::string{str, len});
}
inline TypeSourcePair operator"" _comp(const char* str, size_t len) {
  return std::make_pair(GL_COMPUTE_SHADER, std::string{str, len});
}

/*

        Enabler templates to restruct SetUniform utility function instantiations
   to supported types only.

*/
template <typename T>
using IsValidBaseType =
    std::enable_if_t<std::is_same_v<T, GLint> || std::is_same_v<T, GLuint> ||
...
/*
        ...
        Convert types to OpenGL type GLenum values listed in
   https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glVertexAttribPointer.xhtml

*/

template <typename T>
consteval GLenum NativeTypeToOpenGLType() {
  static_assert(true, "Non-supported vertex attribute type");
  return 0;
}
template <>
consteval GLenum NativeTypeToOpenGLType<float>() {
  return GL_FLOAT;
}
template <>
consteval GLenum NativeTypeToOpenGLType<double>() {
  return GL_DOUBLE;
}

// we have to provide these extraction functions because tvec<*,*>::length(),
// cols(), rows() are not defined as a constexpr...
template <typename T>
constexpr std::pair<size_t, size_t> ComponentCount() {
  return std::make_pair(1, 1);
}
#ifdef GLM_VERSION
#define COMPONENT_COUNT_SPECIALIZATION(TYPE, DIM_X, DIM_Y)     \
  template <>                                                  \
  constexpr std::pair<size_t, size_t> ComponentCount<TYPE>() { \
...
/*
        ...
        Extract primitive type of argument, i.e. what is the base type (e.g.
   float) of whatever construct (e.g. glm::vec3, glm::mat4, etc.) we received.

*/

template <typename T, typename C = void>
struct GLExtractPrimitiveType;
template <typename T>
struct GLExtractPrimitiveType<T, IsValidBaseType<T>> {
  using primitive_type = T;
};
#ifdef GLM_VERSION
template <typename T>
struct GLExtractPrimitiveType<T, IsValidGLMVectorType<T>> {
  using primitive_type = typename T::value_type;
};
template <typename T>
struct GLExtractPrimitiveType<T, IsValidGLMMatrixType<T>> {
  using primitive_type = typename T::value_type;
};
#endif
...
/*
        ...
        Selection of appropriate OpenGL glUniform* function, depending on
   argument primitive type.

*/
template <typename T, int N, int M>
inline void CallSetter(GLint, GLsizei, const T*) {}

#define CALL_SETTER_VECTOR_SPECIALIZATION(TYPE, DIM_X, FUNCTION)     \
  template <>                                                        \
  inline void CallSetter<TYPE, DIM_X, 1>(GLint pLoc, GLsizei pCount, \
                                         const TYPE* pArg) {         \
    FUNCTION(pLoc, pCount, pArg);                                    \
  }
#define CALL_SETTER_MATRIX_SPECIALIZATION(TYPE, DIM_X, DIM_Y, FUNCTION)  \
  template <>                                                            \
  inline void CallSetter<TYPE, DIM_X, DIM_Y>(GLint pLoc, GLsizei pCount, \
                                             const TYPE* pArg) {         \
    FUNCTION(pLoc, pCount, GL_FALSE, pArg);                              \
  }
...
/*
        ...
        Extract the type of the argument elements. In case of a scalar, this
   means the type of the argument. In case of containers this, boils down to the
...
template <typename T>
struct ElementInfo<std::vector<T>> {
  using value_type = T;
};
template <typename T, size_t N>
struct ElementInfo<std::array<T, N>> {
  using value_type = T;
};
template <typename T, size_t N>
struct ElementInfo<T[N]> {
  using value_type = T;
};

/*

        Compute the length of the container in _elements_ in types that our
   SetUniform supports.

*/
template <typename T>
constexpr GLsizei ContainerLength(const T&) {
  return 1;
}
template <typename T, size_t N>
constexpr GLsizei ContainerLength(const std::array<T, N>&) {
  return N;
}
template <typename T, size_t N>
constexpr GLsizei ContainerLength(const T (&)[N]) {
  return N;
}
template <typename T>
constexpr GLsizei ContainerLength(const std::vector<T>& pArr) {
  return (GLsizei)pArr.size();
}
...
constexpr GLsizei ContainerSizeInBytes(const T& range) {
  return sizeof(typename T::value_type) * std::ranges::size(range);
}
template <typename T, size_t N>
constexpr GLsizei ContainerSizeInBytes(const T (&)[N]) {
  return N * sizeof(T);
}
/*

        Get a pointer to the beginning of the container. In case of
   non-contagious containers this should involve the creation of a contagious
   copy of the contents of the passed in containers.

*/
template <typename T>
constexpr const T* PointerToStart(const T& pArg) {
  return &pArg;
}
template <typename T, size_t N>
constexpr const T* PointerToStart(const std::array<T, N>& pArg) {
  return pArg.data();
}
template <typename T, size_t N>
constexpr const T* PointerToStart(const T (&pArg)[N]) {
  return &pArg[0];
}
template <typename T>
constexpr const T* PointerToStart(const std::vector<T>& pArg) {
  return pArg.empty() ? nullptr : pArg.data();
}

/*

        Helper constexpr to determine if the supplied parameter is a supported
   contagious container or not.
...
template <typename T, size_t N>
struct HasContiguousStorage<T[N]> : public std::true_type {};

template <typename T, typename R = void>
using IsContiguousContainer =
    std::enable_if_t<HasContiguousStorage<T>::value, R>;
