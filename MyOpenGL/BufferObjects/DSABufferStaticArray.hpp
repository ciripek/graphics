...
public:
    ...
    DSABufferStaticArray() {
        glCreateBuffers(size, ids.data());
    }

    ~DSABufferStaticArray() {
        glDeleteBuffers(size, ids.data());
    }
    ...
    void storage(GLsizeiptr size, const void* data) const{
        glNamedBufferStorage(ids[index], size, data, static_cast<GLbitfield>(get_usage<index>()));
    }
