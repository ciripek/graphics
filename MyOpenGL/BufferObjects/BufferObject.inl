...
template <BufferType target, BufferUsage usage>
inline BufferObject<target, usage>& BufferObject<target, usage>::operator=(
    BufferObject<target, usage>&& rhs) {
  if (&rhs == this)
    return *this;
...
template <BufferType target, BufferUsage usage>
inline void BufferObject<target, usage>::BufferData(GLsizeiptr pSize,
                                                    const GLvoid* pSource) {
  Bind();

  glBufferData(static_cast<GLenum>(target), pSize, pSource,
               static_cast<GLenum>(usage));
  m_sizeInBytes = pSize;
}

template <BufferType target, BufferUsage usage>
inline void BufferObject<target, usage>::BufferSubData(GLintptr pOffset,
                                                       GLsizeiptr pSize,
                                                       const GLvoid* pSource) {
  Bind();
...
template <BufferType target, BufferUsage usage>
template <typename T>
inline IsContiguousContainer<T> BufferObject<target, usage>::BufferData(
    const T& pArr) {
...
inline BufferObject<target, usage>& BufferObject<target, usage>::operator=(
    const T& pArr) {
  static_assert(HasContiguousStorage<T>::value,
                "Cannot upload data to OpenGL buffer object: supplied argument "
                "is not a known contiguous container.");
...
inline BufferObject<target, usage>::operator std::vector<T>() const {
  ...
  T* ptr =
      static_cast<T*>(glMapBuffer(static_cast<GLenum>(target), GL_READ_ONLY));
