...
ShaderProgram ShaderProgram::fromGLSL(const std::filesystem::path& path,
                                      shaderType type) {
  ...
  const GLuint id =
      glCreateShaderProgramv(static_cast<GLenum>(type), 1, &source);
...
ShaderProgram ShaderProgram::fromSPIRV(const std::filesystem::path& path,
                                       shaderType type) {
  ...
  glShaderBinary(1, &shader, GL_SHADER_BINARY_FORMAT_SPIR_V, source.data(),
                 source.size());
...
void ShaderProgram::cacheUniforms() {
  ...
  if (uniform_count != 0) {
    ...
    for (GLint i = 0; i < uniform_count; ++i) {
      glGetActiveUniform(m_id, i, max_name_len, &length, &count, &type,
                         uniform_name.get());
...
shaderType ShaderProgram::getTypeFromFile(const std::filesystem::path& path) {
  ...
  if (ext == ".comp")
    return shaderType::COMPUTER;
  if (ext == ".spv")
    return getTypeFromFile(path.stem());
  assert(false);
}

void ShaderProgram::setTexture(const std::string& uniform,
                               GLint sampler,
                               GLuint textureID) {
