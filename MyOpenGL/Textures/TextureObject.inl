...
template <textureType type>
inline TextureObject<type>& TextureObject<type>::operator=(
    TextureObject&& rhs) {
  if (&rhs == this)
    return *this;
...
template <textureType type>
inline void TextureObject<type>::AttachFromFile(const std::string& filename,
                                                bool generateMipMap,
                                                GLuint role) {
  SDL_Surface* loaded_img = IMG_Load(filename.c_str());

  if (loaded_img == nullptr) {
    std::cerr << "[AttachFromFile] Error loading image file " << filename
              << std::endl;
    return;
  }

  // Formátum meghatározása és szükség esetén konvertálás
  Uint32 sdl_format = loaded_img->format->BytesPerPixel == 3
  ...
  if (loaded_img->format->format != sdl_format) {
    ...
    if (formattedSurf == nullptr) {
      std::cout << "[AttachFromFile] Error converting image format: "
  ...
  if (type != textureType::TextureCubeMap &&
    ...
    if (transform_image(loaded_img->pitch, loaded_img->h, loaded_img->pixels) ==
        -1) {
      std::cout << "[AttachFromFile] Error transforming image: "
                << SDL_GetError() << std::endl;
      SDL_FreeSurface(loaded_img);
      return;
    }
