...
ProgramObject& ProgramObject::AttachShaders(
    std::initializer_list<ShaderObject> shaderList) {
...
void ProgramObject::BindAttribLocation(int _index,
                                       const char* _inVariableName) const {
  glBindAttribLocation(m_id, _index, _inVariableName);
}

void ProgramObject::BindAttribLocations(
    std::initializer_list<std::pair<int, const char*>> bindingList) const {
  for (const auto& binding : bindingList) {
    glBindAttribLocation(m_id, binding.first, binding.second);
  }
}

void ProgramObject::BindFragDataLocation(int _index,
                                         const char* _outVariableName) const {
  glBindAttribLocation(m_id, _index, _outVariableName);
}

void ProgramObject::BindFragDataLocations(
    std::initializer_list<Binding> fragDataBindingList) const {
...
void ProgramObject::SetTexture(const char* _uniform,
                               int _sampler,
                               GLuint _textureID) {
...
void ProgramObject::SetCubeTexture(const char* _uniform,
                                   int _sampler,
                                   GLuint _textureID) {
  glActiveTexture(GL_TEXTURE0 + _sampler);
